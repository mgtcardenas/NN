...
public class Ring
{
	...
	static Layer			firstLayer	= new Layer(2, 50, new HTan());
	static Layer			secondLayer	= new Layer(50, 1, new Sigmoid());
	...
	private static void evaluateNN(BufferedImage img, double error) throws IOException
	{
		int p, gray;
		for (int y = 0; y < 300; y++) // Go through every one of the 300 by 300
		{
			for (int x = 0; x < 300; x++)
			{
				output	= secondLayer.forward(firstLayer.forward(new double[] { x, y }));
		...
		frame.setTitle("Error: " + error);
	}// end evaluateNN
	
	private static void redrawRings(BufferedImage img)
	{
		for (int i = 0; i < inputs.size(); i++)
			img.setRGB((int) inputs.get(i)[0], (int) inputs.get(i)[1], targets.get(i)[0] == 0 ? 255 << 16 : 255);
	}// end redrawPoints
	...
	private static void setRandomClosePoints(BufferedImage img, int x, int y, int p)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			targets.add(new double[] { color.getRed() == 255 ? 0 : 1 });
		}// end for - i
